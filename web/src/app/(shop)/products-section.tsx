...
function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className="group block bg-card rounded-3xl overflow-hidden border transition-all duration-500 hover:shadow-xl hover:-translate-y-2"
    >
      {/* Image Area */}
      <div className="relative aspect-square bg-muted/50 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5">
          {product.badges.map((badge, idx) => (
            <Badge
              key={idx}
              className={cn(
                "text-xs font-bold px-2.5 py-1 shadow-md",
                badge === "NEW" && "bg-primary text-primary-foreground",
                badge === "BEST" && "bg-primary text-primary-foreground",
                badge.startsWith("-") && "bg-destructive text-destructive-foreground"
              )}
            >
              {badge}
            </Badge>
        ...
        <button
          ...
          className="absolute bottom-3 right-3 z-10 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 bg-gradient-to-r from-[#FFB5C5] to-[#E8A8C8] text-white shadow-lg hover:shadow-xl hover:scale-110"
        >
          <PlusIcon />
        </button>
...
export function ProductsSection() {
  return (
    <section className="py-28 relative overflow-hidden">
      ...
      {/* Decorative Elements */}
      <div className="absolute top-[10%] right-[5%] w-[300px] h-[300px] bg-[#FFE8EE] rounded-full blur-[100px] opacity-40" />
      <div className="absolute bottom-[10%] left-[5%] w-[400px] h-[400px] bg-[#F3E8FF] rounded-full blur-[120px] opacity-30" />
      ...
      <div className="container relative z-10">
        ...
        <div className="text-center mb-16 space-y-4">
          <div className="inline-flex items-center gap-3 rounded-full px-5 py-2 shadow-sm border bg-white/80 backdrop-blur-sm border-[#FFE8EE]">
        ...
        <div className="text-center">
          <Link
            href="/products"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl font-medium transition-all duration-300 hover:-translate-y-1 bg-gradient-to-r from-[#FFB5C5] to-[#E8A8C8] text-white shadow-lg shadow-pink-200/50 hover:shadow-xl"
          >
            View All Products
